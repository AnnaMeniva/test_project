...
//     email: Yup.string().max(250).required ('Email must not exceed 250 characters')
//             .email('Please enter a valid emaeil').required('Email is required'),
    password: Yup.string().min(6).required('Password must be at least 10 characters')
            .max(40) .required('Password must not exceed 30 characters').required('Password is required')
            .matches(
